              ...
              <thead>
                  ...
                  ?>
                      <tr class='odd gradeX context' >
                          ...
                          <td><?php echo $data->notlp_cus?></td>
                          <td><?php echo $data->alamat_cus?></td>
                          <td><?php echo $data->email_cus?></td>
                          <td align="center"> <!-- tambahan "edit" pada class dibawah untuk ajax -->
                           <a class="btn btn-primary edit" data-id="<?php echo $data->id_customer ?>" data-toggle="modal" data-target="#ubahcustomer" class="btn btn-primary btn-xs" class=" btn btn-primary"><i class="fa fa-pencil-square"></i> Edit
                          </a>
                          </td>
                      </tr>
                  <?php 
...
<script type="text/javascript">
  $('.edit').on('click',function(){
    ...
    $.ajax({
      method: "POST",
      url: "<?php echo base_url('index.php/Customer/find/') ?>"+id,
      success: function(customer) {
        var ambil = JSON.parse(customer);
        $('[name="id"]').val(ambil.id_customer);
        $('[name="nama"]').val(ambil.nama_customer);
        $('[name="alamat"]').val(ambil.alamat_cus);
        $('[name="tlp"]').val(ambil.notlp_cus);
        $('[name="email"]').val(ambil.email_cus);
      }
    });
  });
</script>
